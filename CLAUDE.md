...
- **Destructive actions must confirm**: anything that removes a card, a household member or saved data awaits `confirmDialog(msg)` *before* it mutates state. The native-dialog guard above only catches a banned dialog coming back — it cannot see a destructive path that asks nothing at all, which is how three card-removal paths shipped unprompted (fixed in `2227e80`). When adding one, cover it with an e2e test asserting on the **model row**, not on the dialog being visible.
- **Date-only values**: `opened_date`, `closed_date` and `bonus_earned_date` are `DateField`s, and `new Date('2026-09-15')` parses as **UTC** midnight — so `toLocaleDateString()` renders the day before anywhere behind UTC. Use `parseDateOnly(str)` from `static/js/utils.js` for these, never the bare `Date` constructor. Timestamps (`generated_at`) are unaffected.
