...
def generate_recommendations(profile: UserProfile, available_cards: List[CreditCard]) -> Dict[str, Any]:
    ...
    for card in available_cards:
        ...
        value_data = calculate_card_value(card, profile)
    ...
    optimal_combination = find_optimal_card_combination(
        card_values, 
        profile.max_cards, 
    ...
    per_month_value = calculate_value_over_time(optimal_combination, 12)
    
    # Create recommendation data structure
    recommendation_data = {
        'profile_id': profile.id,
        'cards': [card['card_id'] for card in optimal_combination],
        'recommended_sequence': determine_application_sequence(optimal_combination),
...
def find_optimal_card_combination(
    ...
    while remaining_cards and len(current_combination) < max_cards:
        ...
        for i, card in enumerate(remaining_cards):
            ...
            marginal_value = calculate_marginal_value(
                card, 
                current_combination,
                category_spending
            )
    ...
    total_value = calculate_total_combination_value(current_combination, category_spending)
...
def calculate_marginal_value(
    ...
    value_without = calculate_total_combination_value(current_cards, category_spending)
    
    # Create a new combination with the card added
    new_combination = current_cards + [card]
    value_with = calculate_total_combination_value(new_combination, category_spending)
...
def calculate_total_combination_value(
    ...
    # Add the sign-up bonuses and subtract annual fees
    for card in cards:
        total_value += card['signup_bonus_value'] - card['annual_fee']
    ...
    for category, spend in category_spending.items():
        ...
        for card in cards:
            if category in card['category_values']:
                value = card['category_values'][category]
                if value > best_value:
                    best_value = value
        
        # Add the best value for this category (monthly value * 12 for annual)
        total_value += best_value * 12
    
    return total_value

def determine_application_sequence(cards: List[Dict[str, Any]]) -> List[int]:
    """
    Determine the optimal application sequence for cards.
    
    Args:
        cards: List of card value calculations
        
    Returns:
        List of card IDs in the recommended application sequence
    """
    # Sort by signup bonus value (descending)
    sorted_cards = sorted(cards, key=lambda x: x['signup_bonus_value'], reverse=True)
...
def calculate_value_over_time(cards: List[Dict[str, Any]], months: int) -> List[float]:
    """
    Calculate the value of a card combination over time.
    ...
    for month in range(months):
        ...
        # Add monthly value from each card
        for card in cards:
            # Add the card's monthly value
            monthly_value += card['monthly_value']
            
            # Add sign-up bonus if applicable (in the first month)
            if month == 0:
                monthly_value += card['signup_bonus_value']
            
            # Subtract annual fee if applicable (in the first month and annually after)
            if month == 0 or month % 12 == 0:
                monthly_value -= card['annual_fee']
