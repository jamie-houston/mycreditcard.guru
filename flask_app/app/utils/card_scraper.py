...
# Import our specialized NerdWallet scraper
from app.utils.nerdwallet_scraper import scrape_nerdwallet_cards, NerdWalletScraper as SpecializedNerdWalletScraper
...
def scrape_credit_cards(source='nerdwallet', use_proxies=False):
    """
    Main entry point for scraping credit cards from various sources
    
    Args:
        source (str): Source to scrape from (nerdwallet, creditcards.com, etc.)
        use_proxies (bool): Whether to use proxy servers
        
    Returns:
        List of credit card data dictionaries
    """
    logger.info(f"Scraping credit cards from {source}")
    
    # Set up proxy configuration if requested
    proxies = None
    if use_proxies:
